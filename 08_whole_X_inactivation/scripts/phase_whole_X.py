...
def main():
    ...
    gonads_nsites_biased_expn = tabulate_nsites_biased_expn(snakemake.input['gonads'])

    fn=''
    tissues_nsites_biased_expn = {'heart': heart_nsites_biased_expn, 'lung': lung_nsites_biased_expn, 'liver': liver_nsites_biased_expn, 'adrenal': adrenal_nsites_biased_expn, 'pituitary': pituitary_nsites_biased_expn, 'gonads': gonads_nsites_biased_expn}
    ...
    gonads_out = find_ratio_for_hap(snakemake.input['gonads'], pos, hap_1)
    ...
    gonads_out_fn = open(snakemake.output['gonads'], 'w')
    for k, v in gonads_out.items():
        out = [str(k), str(v[0]), str(v[1]), str(v[2])]
        print ("\t".join(out), file=gonads_out_fn)
