...
data = data.frame(tissues=c('gonads', 'lung', 'liver', 'pituitary', 'heart', 'adrenal'), perc=c((1/14)*100, 0, (2/14)*100, 0, 0, (1/14)*100))

data$tissues = as.character(data$tissues)
data$tissues = factor(data$tissues, levels = unique(data$tissues))

png("c://Users/tuyen/Documents/postdoc_asu/projects/Cayo_x_inactivation/compare_distribution_btw_2_chr/chr10_chr16/plots/perc_individuals_allele_balance_differs_AvsA.png", width = 6, height = 4, units = "in", res = 300)
